...
static void calc_activated(GSimpleAction *action, GVariant *parameter,
                           gpointer user_data) {
  GtkWidget *windowCalculator =
      GTK_WIDGET(gtk_builder_get_object(buildCalculate, "win"));
  GtkWidget *windowCredit =
      GTK_WIDGET(gtk_builder_get_object(buildCredit, "windowCredit"));
  gtk_widget_show(windowCalculator);
  gtk_widget_hide(windowCredit);
}

/* метод открытия кредитного калькулятора */
static void credit_activated(GSimpleAction *action, GVariant *parameter,
                             gpointer user_data) {
  GtkWidget *windowCalculator =
      GTK_WIDGET(gtk_builder_get_object(buildCalculate, "win"));
  GtkWidget *windowCredit =
...
static void exit_activated(GSimpleAction *action, GVariant *parameter,
                           gpointer user_data) {
...
const GActionEntry app_entries[] = {
    {"calculator", calc_activated, NULL, NULL, NULL},
...
void print_result_credit(GtkWidget *text, gpointer *data) {
  ...
  GtkWidget *textSum =
      GTK_WIDGET(gtk_builder_get_object(buildCredit, "AmountOfCreditText"));
  GtkWidget *textTerm =
      GTK_WIDGET(gtk_builder_get_object(buildCredit, "CredittermText"));
  GtkWidget *textPercent =
      GTK_WIDGET(gtk_builder_get_object(buildCredit, "InterestRateText"));
  GtkWidget *buttonAnnuity = GTK_WIDGET(
      gtk_builder_get_object(buildCredit, "AnnuityCreditCheckButton"));
  GtkWidget *boxMonthOrYear =
      GTK_WIDGET(gtk_builder_get_object(buildCredit, "creditComboBox"));
  ...
  creditString.sumCredit =
      (char *)gtk_entry_buffer_get_text(gtk_text_get_buffer(GTK_TEXT(textSum)));
  creditString.termCredit = (char *)gtk_entry_buffer_get_text(
      gtk_text_get_buffer(GTK_TEXT(textTerm)));
  creditString.percentCredit = (char *)gtk_entry_buffer_get_text(
      gtk_text_get_buffer(GTK_TEXT(textPercent)));
  creditString.monthOrYear = (char *)gtk_combo_box_text_get_active_text(
      GTK_COMBO_BOX_TEXT(boxMonthOrYear));

  Credit creditRes = credit_work(&creditString);

  GtkWidget *monthlyPayment = GTK_WIDGET(
      gtk_builder_get_object(buildCredit, "MonthlyPaymentAmountText"));
  GtkWidget *overpayment = GTK_WIDGET(
      gtk_builder_get_object(buildCredit, "OverpaymentOnCreditText"));
  GtkWidget *totalPayment =
      GTK_WIDGET(gtk_builder_get_object(buildCredit, "TotalPaymentText"));
  ...
  if (creditRes.check == SUCCESS) {
    if (strcmp("annuity", creditString.typeCredit) == 0)
      ...
      snprintf(resultStr, sizeof(resultStr), "%.2f ... %.2f",
               creditRes.maxMonthlyPayment, creditRes.minMonthlyPayment);
  } else {
    snprintf(resultStr, sizeof(resultStr), "%s", "Error");
  }
  gtk_entry_buffer_set_text(gtk_text_get_buffer(GTK_TEXT(monthlyPayment)),
                            resultStr, strlen(resultStr));
  if (creditRes.check == SUCCESS)
    snprintf(resultStr, sizeof(resultStr), "%.2f", creditRes.overpayment);
  else
    snprintf(resultStr, sizeof(resultStr), "%s", "Error");
  gtk_entry_buffer_set_text(gtk_text_get_buffer(GTK_TEXT(overpayment)),
                            resultStr, strlen(resultStr));
  if (creditRes.check == SUCCESS)
    snprintf(resultStr, sizeof(resultStr), "%.2f", creditRes.interestCharges);
  else
    snprintf(resultStr, sizeof(resultStr), "%s", "Error");
  gtk_entry_buffer_set_text(gtk_text_get_buffer(GTK_TEXT(totalPayment)),
                            resultStr, strlen(resultStr));
}
...
void scale_up(GtkWidget *button, gpointer *data) {
  scaleUp = TRUE;
  gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(button), draw_function, NULL,
                                 NULL);
...
static void window_plot() {
  scaleX = 30;
  scaleY = 30;
  GtkBuilder *build = gtk_builder_new_from_file(
      "/Users/adough/C7_SmartCalc_v1.0-0/src/XML/plot.xml");
  GtkWidget *windowPlot =
      GTK_WIDGET(gtk_builder_get_object(build, "windowPlot"));
...
static void determinate_priority(const char *label, int *valueOperation) {
  if (strcmp(label, "atan") == 0 || strcmp(label, "acos") == 0 ||
      strcmp(label, "asin") == 0) {
    *valueOperation = UOPERATION;
    countOpenBracket++;
  } else if (strcmp(label, "cos") == 0 || strcmp(label, "sin") == 0 ||
             strcmp(label, "tan") == 0) {
    *valueOperation = UOPERATION;
    countOpenBracket++;
  } else if (strcmp(label, "√") == 0 || strcmp(label, "ln") == 0 ||
             strcmp(label, "log") == 0) {
    *valueOperation = UOPERATION;
    countOpenBracket++;
...
static void add_automatic_mult(GtkEntryBuffer *buff) {
  if (preValueOperation == NUMBER) {
    if (valueOperation != OPERATION && valueOperation != NUMBER &&
        valueOperation != CLOSE_BRACKET)
      gtk_entry_buffer_insert_text(buff, gtk_entry_buffer_get_length(buff), "*",
                                   1);
  } else if (preValueOperation == CLOSE_BRACKET ||
             preValueOperation == CONSTANT_OPERATION ||
             preValueOperation == X) {
...
/* проверка операций */
static int check_operation(const char *label, int lenLabel,
                           GtkEntryBuffer *buff) {
  ...
  if (preValueOperation == OPERATION && valueOperation == OPERATION)
    ...
    valueCheck = FAILURE;
  if (valueOperation == NUMBER || valueOperation == OPEN_BRACKET ||
      valueOperation == CONSTANT_OPERATION)
    valueCheck = SUCCESS;
  return valueCheck;
}
...
static void write_text_widget_text(const char *label, int lenLabel,
                                   GtkEntryBuffer *buff) {
  if (strcmp(label, "atan") == 0)
    ...
    gtk_entry_buffer_insert_text(buff, gtk_entry_buffer_get_length(buff), "ln(",
                                 3);
    ...
    gtk_entry_buffer_insert_text(buff, gtk_entry_buffer_get_length(buff), "pi",
                                 2);
  else
    gtk_entry_buffer_insert_text(buff, gtk_entry_buffer_get_length(buff), label,
                                 lenLabel);
}
...
static void load_css() {
  ...
  const gchar *cssStyleFile =
      "/Users/adough/C7_SmartCalc_v1.0-0/src/CSS/style.css";
...
static void activate(GtkApplication *app, gpointer user_data) {
  buildCalculate = gtk_builder_new_from_file(
      "/Users/adough/C7_SmartCalc_v1.0-0/src/XML/calculate.xml");
  GtkWidget *win = gtk_application_window_new(GTK_APPLICATION(app));
  GtkWidget *window = GTK_WIDGET(gtk_builder_get_object(buildCalculate, "win"));
  gtk_window_set_application(GTK_WINDOW(window), GTK_APPLICATION(app));
  buildCredit = gtk_builder_new_from_file(
      "/Users/adough/C7_SmartCalc_v1.0-0/src/XML/credit.xml");
  GtkWidget *windowCredit =
      GTK_WIDGET(gtk_builder_get_object(buildCredit, "windowCredit"));
  gtk_window_set_application(GTK_WINDOW(windowCredit), GTK_APPLICATION(app));
  GtkBuilder *builder = gtk_builder_new_from_file(
      "/Users/adough/C7_SmartCalc_v1.0-0/src/XML/menu.xml");
